...
│   └ Web UI（localhost）：规则编辑/回放/复盘/统计             │
└────────────────────────────────────────────────────────────┘
```
...
| P1 MVP 管道 | 扩展 hook → WS → SQLite → 数条初始规则 → Side Panel 展示分级 | 页面出信号，侧栏秒级出分级 |
| P2 规则系统 | 完整规则引擎 + Web UI 编辑器 + 去重/时间窗口 + 相关性预置包 | 全部规则能力可用 |
| P3 保活 | L0–L3 分层恢复 | 页面挂起 1 分钟内完成恢复或告警 |
| P4 价格追踪 | 自带字段复用或 K 线拉取 → 最高点倍数回填 | 信号自动回填收益倍数 |
| P5 复盘模拟 | 回放器 + 模拟账户 + 质量统计 | 能回答"这套规则赚不赚钱" |
