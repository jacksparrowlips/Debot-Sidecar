...
- 实际根因：安装的 `@fastify/websocket@10.0.1` 回调直接传入 WebSocket，服务端 `/ext` 与 `/ui` 却仍按旧接口读取 `connection.socket`，导致处理器异常、握手后断开。TCP ESTABLISHED 和 HTTP 101 不能证明消息链路正常。
- 修复：两个入口直接使用回调的 `socket`，移除掩盖接口问题的类型断言。
- 验证：修复前真实 `/ext` 客户端先 OPEN 后 CLOSE 1006；新增回归测试修复前超时，修复后通过（扩展上行、扩展/UI 广播）；服务端类型检查通过；当前 Edge Side Panel 已实见「已连接」。
- 尚未验证：真实 DeBot 信号是否完整捕获入库。下文保留原排查记录，其中「WS 实际已连通」为当时未证实的推断。
